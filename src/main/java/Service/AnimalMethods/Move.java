...
public class Move {
    ...
    public void move(Map<Integer, HashMap<Class<? extends Organism>, Integer>> collection, int position, int side, int movementSpeed){
        if (side == 1 && isValidUpMove(position, movementSpeed)) {
            // Up
            goUp(collection, position, movementSpeed);
        } else if (side == 2 && isValidRightMove(position, movementSpeed)) {
            // Right
            goRight(collection, position, movementSpeed);
        } else if (side == 3 && isValidDownMove(position, movementSpeed)) {
            // Down
            goDown(collection, position, movementSpeed);
        } else if (side == 4 && isValidLeftMove(position, movementSpeed)) {
            // Left
            goLeft(collection, position, movementSpeed);
        } else {}
    }
    public void goUp(Map<Integer, HashMap<Class<? extends Organism>, Integer>> collection, int position, int movementSpeed){
        HashMap<Class<? extends Organism>, Integer> mapToDelete = collection.get(position);
        deleteValueCollection(mapToDelete, Organism.class);
        ...
        addValueCollection(mapToAdd, Organism.class);
        collection.put(position - (movementSpeed * 79), mapToAdd);
    }

    public void goRight(Map<Integer, HashMap<Class<? extends Organism>, Integer>> collection, int position, int movementSpeed){
        HashMap<Class<? extends Organism>, Integer> mapToDelete = collection.get(position);
        deleteValueCollection(mapToDelete, Organism.class);
        collection.put(position, mapToDelete);
    ...
    public void goLeft(Map<Integer, HashMap<Class<? extends Organism>, Integer>> collection, int position, int movementSpeed){
        HashMap<Class<? extends Organism>, Integer> mapToDelete = collection.get(position);
        deleteValueCollection(mapToDelete, Organism.class);
        collection.put(position, mapToDelete);
        HashMap<Class<? extends Organism>, Integer> mapToAdd = collection.get(position - 1);
    ...
    public void goDown(Map<Integer, HashMap<Class<? extends Organism>, Integer>> collection, int position, int movementSpeed){
        HashMap<Class<? extends Organism>, Integer> mapToDelete = collection.get(position);
        deleteValueCollection(mapToDelete, Organism.class);
        collection.put(position, mapToDelete);
        HashMap<Class<? extends Organism>, Integer> mapToAdd = collection.get(position + (movementSpeed * 79));
